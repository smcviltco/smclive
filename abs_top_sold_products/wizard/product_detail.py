...
class ProductDetail(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        if data['form']['top_products'] == 'by_units':
            return self.env.ref('abs_top_sold_products.action_report_products').report_action(self, data=data, config=False)
        else:
            return self.env.ref('abs_top_sold_products.action_report_products_amount').report_action(self, data=data, config=False)
