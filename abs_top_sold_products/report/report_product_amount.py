...
class ReportProductsAmount(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for product_id, price_subtotal in sorted(product_records.items(), key=lambda kv: kv[1], reverse=True)[:docs.no_of_products]:
            sorted_product_records.append({'name':product_id.name, 'amount': int(price_subtotal), 'pricelist_id' : self.env.user.company_id.currency_id })
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'docs': docs,
