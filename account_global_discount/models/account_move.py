...
class AccountMove(models.Model):
    ...
    global_discount_ids = fields.Many2one(
    ...
    def _compute_amount_one(self):
        if not self.invoice_global_discount_ids:
            ...
            return
        round_curr = self.currency_id.round
        self.amount_global_discount = sum( round_curr(discount.discount_amount) * -1  for discount in self.invoice_global_discount_ids )
        total=sum( round_curr(discount.discount_amount) * -1  for discount in self.invoice_global_discount_ids )
        self.amount_global_discount=total
        print(total)
...
class AccountInvoiceGlobalDiscount(models.Model):
    ...
    global_discount_id = fields.Many2one(comodel_name="global.discount", string="Global Discount", readonly=True)
    discount = fields.Float(string="Discount (number)")
    discount_display = fields.Char(compute="_compute_discount_display", readonly=True, string="Discount")
    base = fields.Float(string="Base discounted", digits="Product Price", readonly=True)
    base_discounted = fields.Float(string="Discounted amount", digits="Product Price", readonly=True)
    currency_id = fields.Many2one(related="invoice_id.currency_id", readonly=True)
    discount_amount = fields.Monetary(string="Discounted Amount", compute="_compute_discount_amount",currency_field="currency_id",readonly=True,compute_sudo=True)
